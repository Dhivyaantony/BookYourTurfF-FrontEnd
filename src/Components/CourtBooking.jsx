...
function CourtBook() {
    ...
    const getTimeSlotData = (date = new Date()) => {
        AxiosInstance.get('/users/dayWiseTimeSlot', { params: { courtId: id, date: date } }).then((res) => {
            ...
            console.log("Slot data:", res.data); // Add console.log here to log the slot data
        }).catch((err) => {
            console.log(err);
        });
    };
    ...
    const initiateBooking = async () => {
        try {
            ...
            if (!res) {
                // Handle failure to load Razorpay SDK
            ...
            // Define options for Razorpay payment modal
            const options = {
                key: "rzp_test_MjyoFZ3GHvHbzw", // Your Razorpay Key ID
                ...
                handler: async function (response) {
                    ...
                    const successResponse = await AxiosInstance.post("/payment/paymentSuccess", data);
                    if (successResponse && successResponse.data) {
                        // Handle success response from server
                        toastSuccess(successResponse.data.msg);
                        setBookingModal(false);
                        getTimeSlotData(new Date(inputDate));
                    } else {
                        // Handle invalid or missing response from server
                        alert("Failed to process payment success. Please try again later.");
                    }
                },
    ...
    const handleEditCourt = () => {
        setCourtEditModal(true); // Open the edit modal
    };
...
getSinglecourtData()
    ...
    const handleNavigation = (path) => {
        if (path === 'today') {
            setInputDate(new Date().toISOString().split('T')[0]); // Set inputDate to today's date
        } else if (path === 'tomorrow') {
            const tomorrow = new Date();
            tomorrow.setDate(tomorrow.getDate() + 1); // Increment by 1 day for tomorrow
            setInputDate(tomorrow.toISOString().split('T')[0]); // Set inputDate to tomorrow's date
        }
        // Other navigation logic...
    };
    const handleEditTimeSlot = (slotId) => {
        // Implement logic to edit the details of the selected time slot
    };
    
    const handleDeleteTimeSlot = (slotId) => {
        // Implement logic to delete the selected time slot from the list of selected time slots
        setSelectedTimings(selectedTimings.filter(slot => slot.id !== slotId));
        // You might also want to remove the associated cost when deleting a time slot
        setTimeSlotData(prevState => {
            const updatedTimeSlotData = { ...prevState };
            delete updatedTimeSlotData[slotId];
            return updatedTimeSlotData;
        });
    };
    //scrolling text
    return (
        <>
         <div className="d-flex">
                ...
                >
                    ...
                    <div>{singleCourtData.courtName}</div>
        ...
        <div className="add-edit-buttons">
            <button  className="btn btn-black text-light border rounded-2" onClick={() => setModalOpen(true)}>Add Time Slot</button>
            <button className="btn btn-black text-light border rounded-2" onClick={() => setCourtEditModal(true)}>Edit Court</button>
        </div>
    )}
    ...
    <div className="single-court-img-box">
        ...
        <div className="court-name">
            <span>{singleCourtData.courtName}</span>
        </div>
    </div>

            <div className="container-fluid m1 mt-5 ">
                <div className="row justify-content-center">
                    <div className="col-md-6 col-lg-4 col-12 border border-1 rounded-2 m2">
                        <div className="date-picker">
                        <div >
              <span> <button onClick={() => handleNavigation('today')}className="date-btn">Today</button>
                <button onClick={() => handleNavigation('tomorrow')}className="date-btn tom">Tomorrow</button></span>
            </div>
                            <div>
                        ...
                        <button className="btn btn-primary w-100 mt-5 border mb-5">
                            {" "}
                            Book Now{" "}
                        </button>
                    </div>
            ...
            <p><strong>Court Name:</strong> {selectedSlot?.court?.courtName}</p>
    ...
    <button onClick={initiateBooking}>Book now</button>
            ...
            <input type="number" id="cost" name="cost" value={editCourtData.cost} onChange={handleCourtDataChg} />
